...
| 8 | iPhone 上传照片报 `扫描出错 End of input`（JSONToken） | Kotlin `UploadBridgeModule`（MODULE_NAME="HRBridgeModule"）调 `scanGallery/scanUpload/scanPoll/copyOriginal/batchUpload`，iOS 宿主 KRBridgeModule.m 未实现 → 同步方法返回空 → `JSONObject("")` 抛 End of input | KRBridgeModule.m 实现 5 方法（PHPhotoLibrary 扫描、缩略图导出 file://、photoId↔localIdentifier 映射、NSURLSession multipart 上传 zhuyanyou.fun、NSUserDefaults 去重）+ Info.plist 加 NSPhotoLibraryUsageDescription + project.yml `sdk: Photos.framework`。踩坑：同步 GET 用 dispatch_semaphore（NSURLSession 无 sendSynchronousRequest）；xcodegen 系统框架用 `sdk:` 不是 `framework:`（commits 79e2528→ef0589c→0a6b18a→2708d9f） |

**✅ 当前状态（2026-08-08 已全绿）**：1~8 关全过。最新 run **31233228456（commit 2708d9f）conclusion=success**：`BUILD SUCCEEDED` → `ChaijieApp-unsigned.ipa`（5.3MB）→ artifact `ChaijieApp-unsigned-ipa`（ID 9014637934）。**该 ipa 含 iOS 原生上传功能（相册扫描/人脸分组/批量上传），已拉回本机 `E:/kuikly_apps/ChaijieApp-iOS/dist/ChaijieApp-unsigned.ipa`**。剩余：Sideloadly 重装真机验证上传；可选付费证书签名。
